...
task GetVcfSize {
    ...
    command <<<
        ...
        INDICES_LIST=~{write_lines(select_all([vcf_index]))}
        if [ -s $INDICES_LIST ]; then
            # if index exists at this path, copy it to current working dir
            if gsutil -q stat ~{automatic_vcf_index}; then
                gsutil cp ~{automatic_vcf_index} .
            fi
        else
            # vcf index was supplied, symlink it to current working dir
            read INDEX_FILE < $INDICES_LIST
            ln -s "$INDEX_FILE" .
        fi

        export GCS_OAUTH_TOKEN=`gcloud auth application-default print-access-token`
        bcftools query -l ~{vcf} | wc -w > ~{num_samples_file}
        # get num records from index. If index is wrong kind or missing, build index on the fly.
        {
